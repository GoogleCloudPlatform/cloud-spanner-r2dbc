...
final class ArrayInt64Codec extends AbstractCodec<long[]> {
    ...
    boolean doCanDecode(Type dataType) {
        return dataType.equals(Type.array(Type.int64()));
    }

    @Override
    long[] doDecode(Struct row, int index, Class<? extends long[]> type) {
        return row.getLongArray(index);
    }

    @Override
    Value doEncode(long[] value) {
        return Value.int64Array(value);
    }
}
