...
final class ArrayTimestampCodec extends AbstractCodec<Timestamp[]> {
    ...
    Timestamp[] doDecode(Struct row, int index, Class<? extends Timestamp[]> type) {
        return row.getTimestampList(index).toArray(new Timestamp[0]);
    }

    @Override
    Value doEncode(Timestamp[] value) {
        return Value.timestampArray(value == null ? null : Arrays.asList(value));
    }
}
